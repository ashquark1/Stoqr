...
## 2026-06-12: US-13 — status visibility moves to the store (revises US-03)

- Decision: the default Active-only filter moves OUT of `SheetsData.results` into `ProductSearchStore.statusVisible` (`applyStatusVisibility` over `sheets.products()`), because the AC-08–12 reveal toggles need the full set — anything dropped at the mapping/data layer could never be un-hidden. This REVISES the 2026-06-10 US-03 decision ("Active filter lives in SheetsData.results"); `results` remains as the Active baseline used by `isEmpty`, but the store pipeline now derives from `statusVisible`, so options + count exclude hidden rows automatically (AC-06/07).
- Decision (AC-04 reinterpreted): status filtering stays in the CORE layer, before any component — NOT in `DataMapping` (which stays pure/complete). Same reasoning the US-03 entry already used to reject the tech-note's "data mapping layer" wording.
- Decision (toggles): `showInactive`/`showDiscontinued` live in `FilterService`, separate from facet selections — they relax the default (not narrow), so they are excluded from `activeCount`. Default off; reset by `resetStatusToggles()`. Reset is driven by an effect in the store watching `SheetsData.fetchedAt` — every successful new-search/refresh fetch turns them off (AC-12); local typing within a session doesn't fetch, so it preserves them. `reset()` (Reset Filters) also clears them.
- Decision (product status as a domain): new `core/product-status.ts` (resolver + pure visibility filter), distinct from `core/stock-status.ts`.
- Decision (badges): `<stoqr-status-badge>` is a distinct visual family from the stock badge (`.stoqr-status-badge`, outlined, no dot) to satisfy AC-13, while Inactive uses the grey Unknown hue and Discontinued a muted (outlined) red.
- Decision (toggle UI): native `<stoqr-toggle>` primitive (role=switch), no vendor dependency — consistent with the other native primitives.

## 2026-06-12: US-12 — responsive audit (patterns + test approach)

- Decision (AC-15 testing): keep Vitest+jsdom; assert breakpoint-driven DOM CONTRACTS (component structure/classes) rather than computed CSS, which jsdom can't evaluate. Real pixel/layout verification is done by driving the running app at each breakpoint (Launch preview). Rejected adding Playwright/Cypress — a new test framework wasn't justified for V1 (Q-G2).
- Decision (modal): full-screen on mobile, 520px centered card from tablet up — in `_primeng-overrides.scss` scoped to `.stoqr-modal-dialog` (overlay-rendered), using the `from(tablet)` mixin; mobile uses literal `0` for border/radius/margin (no `--radius-none` token).
- Decision (results toolbar): a regrouping CSS grid tied to the button icon-only breakpoint (tablet). Mobile (icon-only Filters + Refresh): 2×2 grid — both buttons on the top row, count + Last updated below. From tablet (buttons show text): a single row — Filters + count on the left, Last updated + Refresh on the right edge (the original Sprint-1 grouping). The Filters label is wrapped in `.icon-btn__label` so it collapses to icon-only with Search/Refresh.
- Decision (icon-only buttons): Search, Refresh, and Filters all reuse the shared `.icon-btn` class and collapse to a **44×44 square** (hero Search 52×52) with an 18px icon when icon-only on mobile, widening to `width:auto` for the label from tablet up. Filters dropped its one-off size/icon override to join the shared class (reuse over duplication — see the new Q-G3 rule in AI_CONTRACT).
- Decision (tap targets): ≥44px on mobile for the status toggles, filter-select triggers, and Reset; relaxed to compact density from tablet up. The paginator page/nav buttons are 44px **at every width** (uniform, per user request) — circular via `border-radius:50%`.
- Decision (pagination layout): two rows on tablet and smaller (`< desktop`) — current-page report + page-size selector share the top line, page buttons centered below; from desktop up a single inline row — report at the left, page buttons in the TRUE center (a `1fr repeat(5,auto) 1fr` grid so the equal side columns centre the buttons regardless of side widths), selector at the right. The page-size selector matches the button height (44px) with its value centred, at every width.
- Decision (shared roundedness): one semantic token `--radius-control` (= `--radius-xl`, 10px) drives the corner radius of every visibly-bordered rectangular surface (search bar, buttons, table, filter panel, dropdowns + overlay, page-size selector, toast, modal). Pills/circles opt out. The modal also needs `overflow:hidden` so the header background clips to the rounded corners. Change the one alias to restyle all of them.
- Decision (modal corners): `.stoqr-modal-dialog` gets `overflow:hidden` so the paper header is clipped to the radius on the tablet+ card (no effect on the mobile full-screen state).

## 2026-06-13: US-14 — sort search results (custom, not PrimeNG)

- Decision (custom sort): sort is OUR own pure function (`core/sort/product-sort.ts` `sortProducts`) applied in `ProductSearchStore.sortedProducts` (pipeline `… active filters → sort → paginate`), with state in a dedicated `SortService` (`core/sort/`). Rejected PrimeNG `pSortableColumn`/built-in sort — we need the 3-state cycle (PrimeNG is 2-state), a 2-column allow-list, "logic in services", and an independently unit-testable pure function. PrimeNG's paginator still pages the already-sorted array, so sort persists across pages for free (AC-09).
- Decision (3-state, single column): `SortService.cycle(column)` → a new column starts asc; the active column goes asc → desc → none (AC-04/07).
- Decision (compare rules): Product Name = `localeCompare` `sensitivity:'base'` (case-insensitive); Stock Qty = numeric; empty stock (`null`) is pinned LAST in both directions (the null check precedes the direction factor, so it never reads as a real low/high qty); stable sort keeps sheet order for ties; `null` sort = sheet order (AC-13).
- Decision (persist vs reset — the key one): sort must persist on Refresh (AC-11) but reset on a NEW search (AC-12). The US-13 toggle-reset (effect on `fetchedAt`) fires on every fetch incl. refresh, so it can't be reused. Added `SheetsData.searchSerial` — a counter incremented only when a search SESSION STARTS (`searchNow`/`onSearchInput` threshold), not on `refresh()`. A store effect resets the sort off `searchSerial`. Within-session local query refinement doesn't tick it, so it preserves the sort.
- Decision (independent of filters): `SortService` is separate from `FilterService`; "Reset Filters" does NOT clear the sort (sort isn't a filter).
- Decision (wrapper seam): `<stoqr-data-table>` gains a `sortable` flag on `ColumnDef`, a `sort` input, and a `sortChange` output, rendering clickable headers + indicators (neutral double-arrow when unsorted, up/down when active). Sorting is driven by the store, not PrimeNG — features stay vendor-agnostic.
